/**
 * Controller Layer — Async HTTP Handlers
 *
 * KEY CONCEPT: Since services now talk to a database (async I/O),
 * every handler must be async with try/catch. If a database query fails,
 * we forward the error to Express's error handler via next(error) instead
