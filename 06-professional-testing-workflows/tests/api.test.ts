...
describe('Module 06: Professional Testing Workflows', () => {
  ...
  beforeAll(async () => {
    // 1. Start an isolated PostgreSQL container for this specific test suite run
    ...
    // 3. Push the schema to the isolated test database
    console.log('📦 Pushing Drizzle Schema to Testcontainer...');
    execSync('npx drizzle-kit push', { 
      env: { ...process.env, DATABASE_URL: uri },
      stdio: 'ignore' // hide drizzle-kit output during tests
    });

    // 4. Dynamically import our app modules so they correctly bind to the new process.env
    // @ts-ignore
    ...
    // 5. Shared Setup: Create admin and customer accounts
  ...
  }, 60000); // Allow 60s for downloading the postgres image
  ...
  describe('Authentication Endpoints', () => {
    it('should register a new user', async () => {
      const res = await request.post('/api/auth/register').send({ username: 'newuser', password: 'password123' });
      expect(res.status).toBe(201);
      expect(res.body.username).toBe('newuser');
    });

    it('should not register duplicate users', async () => {
      const res = await request.post('/api/auth/register').send({ username: 'newuser', password: 'password' });
      expect(res.status).toBe(409);
    });

    it('should login an existing user', async () => {
      const res = await request.post('/api/auth/login').send({ username: 'newuser', password: 'password123' });
  ...
  describe('Secured Book Endpoints', () => {
    ...
    it('should allow anyone to read books', async () => {
      const res = await request.get('/api/books');
      expect(res.status).toBe(200);
    });

    it('should block anonymous users from creating books', async () => {
      const res = await request.post('/api/books').send(newBook);
      expect(res.status).toBe(401);
    });

    it('should block customers from creating books', async () => {
      const res = await request.post('/api/books').set('Authorization', `Bearer ${customerToken}`).send(newBook);
      expect(res.status).toBe(403);
    });

    it('should allow admins to create books', async () => {
      const res = await request.post('/api/books').set('Authorization', `Bearer ${adminToken}`).send(newBook);
      expect(res.status).toBe(201);
    });

    it('should allow admins to delete books', async () => {
      const res = await request.delete(`/api/books/${book1Id}`).set('Authorization', `Bearer ${adminToken}`);
