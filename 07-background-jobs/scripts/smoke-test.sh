#!/usr/bin/env bash
# Module 07: Smoke Test — Server + Worker (dual process)
...
echo -e "${CYAN}Module 07: Background Jobs — Smoke Test${NC}"
echo ""
...
http_post "$BASE/api/auth/register" '{"username":"admin07","password":"password123"}' > /dev/null
docker compose exec -T db psql -U user -d bookstore -c "UPDATE users SET role='admin' WHERE username='admin07';" > /dev/null 2>&1

RESULT=$(http_post "$BASE/api/auth/login" '{"username":"admin07","password":"password123"}')
ADMIN_TOKEN=$(extract_json_field "$(echo "$RESULT" | cut -d'|' -f2-)" "token")

http_post "$BASE/api/auth/register" '{"username":"customer07","password":"password123"}' > /dev/null
RESULT=$(http_post "$BASE/api/auth/login" '{"username":"customer07","password":"password123"}')
...
if [ -n "$CUSTOMER_TOKEN" ] && [ "$CUSTOMER_TOKEN" != "null" ]; then
  pass "Customer registered and logged in"
else
  fail "Customer registered and logged in"
fi

# ── Create a book as admin ──────────────────────────────────────────
RESULT=$(http_post "$BASE/api/books" '{"title":"Worker Test Book","author":"Tester","pages":200,"published":"2024"}' "Bearer $ADMIN_TOKEN")
...
# ── Place an order as customer → 202 Accepted ──────────────────────
...
if [ "$INITIAL_STATUS" = "pending" ]; then
  pass "Order initial status is 'pending'"
else
  fail "Order initial status is 'pending' (got: $INITIAL_STATUS)"
fi
...
if [ -n "$ORDER_ID" ] && [ "$ORDER_ID" != "null" ]; then
  ...
  if [ "$FINAL_STATUS" = "completed" ]; then
    pass "Order status changed to 'completed' after worker processed"
  else
    fail "Order status changed to 'completed' (got: $FINAL_STATUS)"
  fi
else
  fail "Could not extract order ID to verify worker processing"
fi
