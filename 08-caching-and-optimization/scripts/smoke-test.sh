#!/usr/bin/env bash
# Module 08: Smoke Test — Server + Worker (with caching)
...
echo -e "${CYAN}Module 08: Caching and Optimization — Smoke Test${NC}"
echo ""
...
http_post "$BASE/api/auth/register" '{"username":"admin08","password":"password123"}' > /dev/null
docker compose exec -T db psql -U user -d bookstore -c "UPDATE users SET role='admin' WHERE username='admin08';" > /dev/null 2>&1

RESULT=$(http_post "$BASE/api/auth/login" '{"username":"admin08","password":"password123"}')
ADMIN_TOKEN=$(extract_json_field "$(echo "$RESULT" | cut -d'|' -f2-)" "token")

http_post "$BASE/api/auth/register" '{"username":"customer08","password":"password123"}' > /dev/null
RESULT=$(http_post "$BASE/api/auth/login" '{"username":"customer08","password":"password123"}')
...
RESULT=$(http_post "$BASE/api/books" '{"title":"Cache Test Book","author":"Tester","pages":200,"published":"2024"}' "Bearer $ADMIN_TOKEN")
