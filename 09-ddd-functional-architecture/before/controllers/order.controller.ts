/**
 * Order Controller — The "Before DDD" Version
 *
 * REFACTORING EXERCISE: Notice how this controller:
 *   1. Defines its own Zod schema (should be in domain/order/order.types.ts)
 *   2. Validates input inline (should be in the use case)
 *   3. Calls the service directly (should call a use case instead)
 *
 * YOUR TASK: Make this controller "thin" — it should only:
 *   - Extract data from req (params, body, user)
 *   - Call placeOrderUseCase() or getOrderUseCase()
 *   - Map DomainError → 400, unexpected error → next(error)
 */
