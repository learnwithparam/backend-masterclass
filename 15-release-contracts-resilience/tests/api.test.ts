...
describe('Module 15: Release Contracts and Resilience', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      process.env.INVENTORY_PORT = '4150';
  ...
  describe('Inter-Service Communication (REST -> Pub/Sub -> DB)', () => {
    ...
    it('publishes the OpenAPI contract and supports failure drills', async () => {
      const contract = await request.get('/openapi.json');
      expect(contract.status).toBe(200);
      expect(contract.body.openapi).toBe('3.0.3');
      expect(contract.body.paths['/api/orders']).toBeDefined();

      const drill = await request.get('/api/chaos?mode=dependency');
      expect(drill.status).toBe(503);
      expect(drill.body.error).toContain('Downstream dependency unavailable');
    });
  });
});
